...
class Chapter9ModelTests(TestCase):
    ...
    def test_userprofile_class(self):
        ...
        self.assertEqual(found_count, len(expected_attributes.keys()),
                         f"{FAILURE_HEADER}In the UserProfile model, we found {found_count} attributes, but were expecting {len(expected_attributes.keys())}. Check your implementation and try again.{FAILURE_FOOTER}")
        user_profile.save()
    ...
    def test_model_admin_interface_inclusion(self):
        ...
        self.assertEqual(response.status_code, 200,
                         f"{FAILURE_HEADER}When attempting to access the UserProfile in the admin interface, we didn't get a HTTP 200 status code. Did you register the new model with the admin interface?{FAILURE_FOOTER}")
...
class Chapter9RegisterFormClassTests(TestCase):
    ...
    def test_user_profile_form(self):
        ...
        self.assertTrue('UserProfileForm' in dir(forms),
                        f"{FAILURE_HEADER}We couldn't find the UserProfileForm class in Rango's forms.py module. Did you create it in the right place?{FAILURE_FOOTER}")

        user_profile_form = forms.UserProfileForm()
        self.assertEqual(type(user_profile_form.__dict__['instance']), rango.models.UserProfile,
                         f"{FAILURE_HEADER}Your UserProfileForm does not match up to the UserProfile model. Check your Meta definition of UserProfileForm and try again.{FAILURE_FOOTER}")
        ...
        for expected_field_name in expected_fields:
            ...
            self.assertTrue(expected_field_name in fields.keys(),
                            f"{FAILURE_HEADER}The field {expected_field_name} was not found in the UserProfile form. Check you have complied with the specification, and try again.{FAILURE_FOOTER}")
            self.assertEqual(expected_field, type(fields[expected_field_name]),
...
class Chapter9RegistrationTests(TestCase):
    ...
    def test_new_registration_view_exists(self):
        ...
        self.assertEqual(url, '/rango/register/',
                         f"{FAILURE_HEADER}Have you created the rango:register URL mapping correctly? It should point to the new register() view, and have a URL of '/rango/register/' Remember the first part of the URL (/rango/) is handled by the project's urls.py module, and the second part (register/) is handled by the Rango app's urls.py module.{FAILURE_FOOTER}")
    ...
    def test_registration_template(self):
        ...
        self.assertTrue(os.path.exists(template_path),
                        f"{FAILURE_HEADER}We couldn't find the 'register.html' template in the 'templates/rango/' directory. Did you put it in the right place?{FAILURE_FOOTER}")
        ...
        self.assertTrue(re.search(full_title_pattern, content),
                        f"{FAILURE_HEADER}The <title> of the response for 'rango:register' is not correct. Check your register.html template, and try again.{FAILURE_FOOTER}")
        self.assertTrue(re.search(block_title_pattern, template_str),
                        f"{FAILURE_HEADER}Is register.html using template inheritance? Is your <title> block correct?{FAILURE_FOOTER}")
    ...
    def test_registration_get_response(self):
        ...
        self.assertTrue('<h1>Register for Rango</h1>' in content,
                        f"{FAILURE_HEADER}We couldn't find the '<h1>Register for Rango</h1>' header tag in your register template. Did you follow the specification in the book to the letter?{FAILURE_FOOTER}")
        self.assertTrue('Rango says: <strong>register here!</strong>' in content,
                        f"{FAILURE_HEADER}When loading the register view with a GET request, we didn't see the required 'Rango says: <strong>register here!</strong>'. Check your register.html template and try again.{FAILURE_FOOTER}")
        self.assertTrue('enctype="multipart/form-data"' in content,
                        f"{FAILURE_HEADER}In your register.html template, are you using 'multipart/form-data' for the <form>'s 'enctype'?{FAILURE_FOOTER}")
        self.assertTrue('action="/rango/register/"' in content,
                        f"{FAILURE_HEADER}Is your <form> in register.html pointing to the correct URL for registering a user?{FAILURE_FOOTER}")
        self.assertTrue('<input type="submit" name="submit" value="Register" />' in content,
                        f"{FAILURE_HEADER}We couldn't find the markup for the form submission button in register.html. Check it matches what is in the book, and try again.{FAILURE_FOOTER}")
        self.assertTrue(
            '<p><label for="id_password">Password:</label> <input type="password" name="password" required id="id_password"></p>' in content,
            f"{FAILURE_HEADER}Checking a random form field in register.html (password), the markup didn't match what we expected. Is your password form field configured correctly?{FAILURE_FOOTER}")
    ...
    def test_good_form_creation(self):
        ...
        user_profile_data = {'website': 'http://www.bing.com',
                             'picture': tempfile.NamedTemporaryFile(suffix=".jpg").name}
        ...
        self.assertEqual(len(rango.models.UserProfile.objects.all()), 1,
                         f"{FAILURE_HEADER}We were expecting to see a UserProfile object created, but it didn't appear. Check your UserProfileForm implementation, and try again.{FAILURE_FOOTER}")
    ...
    def test_good_registration_post_response(self):
        ...
        """
        post_data = {'username': 'webformuser', 'password': 'test123', 'email': 'test@test.com',
        ...
        self.assertTrue('<h1>Register for Rango</h1>' in content,
                        f"{FAILURE_HEADER}We were missing the '<h1>Register for Rango</h1>' header in the registration response.{FAILURE_FOOTER}")
        self.assertTrue('Rango says: <strong>thank you for registering!</strong>' in content,
                        f"{FAILURE_HEADER}When a successful registration occurs, we couldn't find the expected success message. Check your implementation of register.html, and try again.{FAILURE_FOOTER}")
        self.assertTrue('<a href="/rango/">Return to the homepage.</a>' in content,
                        f"{FAILURE_HEADER}After successfully registering, we couldn't find the expected link back to the Rango homepage.{FAILURE_FOOTER}")
...
class Chapter9LoginTests(TestCase):
    ...
    def test_login_functionality(self):
        ...
        try:
            self.assertEqual(user_object.id, int(self.client.session['_auth_user_id']),
                             f"{FAILURE_HEADER}We attempted to log a user in with an ID of {user_object.id}, but instead logged a user in with an ID of {self.client.session['_auth_user_id']}. Please check your login() view.{FAILURE_FOOTER}")
        except KeyError:
            self.assertTrue(False,
                            f"{FAILURE_HEADER}When attempting to log in with your login() view, it didn't seem to log the user in. Please check your login() view implementation, and try again.{FAILURE_FOOTER}")

        self.assertEqual(response.status_code, 302,
                         f"{FAILURE_HEADER}Testing your login functionality, logging in was successful. However, we expected a redirect; we got a status code of {response.status_code} instead. Check your login() view implementation.{FAILURE_FOOTER}")
        self.assertEqual(response.url, reverse('rango:index'),
                         f"{FAILURE_HEADER}We were not redirected to the Rango homepage after logging in. Please check your login() view implementation, and try again.{FAILURE_FOOTER}")
    ...
    def test_login_template(self):
        ...
        self.assertTrue(re.search(block_title_pattern, template_str),
                        f"{FAILURE_HEADER}Is login.html using template inheritance? Is your <title> block correct?{FAILURE_FOOTER}")
    ...
    def test_login_template_content(self):
        ...
        template_str = get_template(template_path)
        self.assertTrue('<h1>Login to Rango</h1>' in template_str,
                        f"{FAILURE_HEADER}We couldn't find the '<h1>Login to Rango</h1>' in the login.html template.{FAILURE_FOOTER}")
        self.assertTrue('action="{% url \'rango:login\' %}"' in template_str,
                        f"{FAILURE_HEADER}We couldn't find the url lookup for 'rango:login' in your login.html <form>.{FAILURE_FOOTER}")
        self.assertTrue('<input type="submit" value="submit" />' in template_str,
                        f"{FAILURE_HEADER}We couldn't find the submit button in your login.html template. Check it matches what is in the book, and try again.{FAILURE_FOOTER}")
    ...
    def test_homepage_greeting(self):
        ...
        self.assertTrue('hey there partner!' in content,
                        f"{FAILURE_HEADER}We didn't see the generic greeting for a user not logged in on the Rango homepage. Please check your index.html template.{FAILURE_FOOTER}")

        create_user_object()
        self.client.login(username='testuser', password='testabc123')

        content = self.client.get(reverse('rango:index')).content.decode()
        self.assertTrue('howdy testuser!' in content,
                        f"{FAILURE_HEADER}After logging a user, we didn't see the expected message welcoming them on the homepage. Check your index.html template.{FAILURE_FOOTER}")
...
class Chapter9RestrictedAccessTests(TestCase):
    ...
    def test_restricted_url_exists(self):
        ...
        self.assertEqual(url, '/rango/restricted/',
                         f"{FAILURE_HEADER}Have you created the rango:restricted URL mapping correctly? It should point to the new restricted() view, and have a URL of '/rango/restricted/' Remember the first part of the URL (/rango/) is handled by the project's urls.py module, and the second part (restricted/) is handled by the Rango app's urls.py module.{FAILURE_FOOTER}")
    ...
    def test_bad_request(self):
        ...
        self.assertEqual(response.status_code, 302,
                         f"{FAILURE_HEADER}We tried to access the restricted view when not logged in. We expected to be redirected, but were not. Check your restricted() view.{FAILURE_FOOTER}")
        self.assertTrue(response.url.startswith(reverse('rango:login')),
                        f"{FAILURE_HEADER}We tried to access the restricted view when not logged in, and were expecting to be redirected to the login view. But we were not! Please check your restricted() view.{FAILURE_FOOTER}")
...
class Chapter9LogoutTests(TestCase):
    ...
    def test_good_request(self):
        ...
        try:
            self.assertEqual(user_object.id, int(self.client.session['_auth_user_id']),
                             f"{FAILURE_HEADER}We attempted to log a user in with an ID of {user_object.id}, but instead logged a user in with an ID of {self.client.session['_auth_user_id']}. Please check your login() view. This happened when testing logout functionality.{FAILURE_FOOTER}")
        except KeyError:
            self.assertTrue(False,
                            f"{FAILURE_HEADER}When attempting to log a user in, it failed. Please check your login() view and try again.{FAILURE_FOOTER}")

        # Now lot the user out. This should cause a redirect to the homepage.
        response = self.client.get(reverse('rango:logout'))
        self.assertEqual(response.status_code, 302,
                         f"{FAILURE_HEADER}Logging out a user should cause a redirect, but this failed to happen. Please check your logout() view.{FAILURE_FOOTER}")
        self.assertEqual(response.url, reverse('rango:index'),
                         f"{FAILURE_HEADER}When logging out a user, the book states you should then redirect them to the homepage. This did not happen; please check your logout() view.{FAILURE_FOOTER}")
        self.assertTrue('_auth_user_id' not in self.client.session,
                        f"{FAILURE_HEADER}Logging out with your logout() view didn't actually log the user out! Please check yout logout() view.{FAILURE_FOOTER}")
...
class Chapter9LinkTidyingTests(TestCase):
    ...
    def test_omnipresent_links(self):
        ...
        content = self.client.get(reverse('rango:index')).content.decode()
        self.assertTrue('href="/rango/about/"' in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/rango/"' in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
    ...
    def test_logged_in_links(self):
        ...
        # These should be present.
        self.assertTrue('href="/rango/restricted/"' in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/rango/logout/"' in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")

        # These should not be present.
        self.assertTrue('href="/rango/login/"' not in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/rango/register/"' not in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
    ...
    def test_logged_out_links(self):
        ...
        # These should be present.
        self.assertTrue('href="/rango/login/"' in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/rango/register/"' in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")

        # These should not be present.
        self.assertTrue('href="/rango/restricted/"' not in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
        self.assertTrue('href="/rango/logout/"' not in content,
                        f"{FAILURE_HEADER}Please check the links in your base.html have been updated correctly to change when users log in and out.{FAILURE_FOOTER}")
...
class Chapter9ExerciseTests(TestCase):
    ...
    def test_restricted_template_exists(self):
        ...
        self.assertTrue(os.path.exists(template_path),
                        f"{FAILURE_HEADER}We couldn't find the 'restricted.html' template in the 'templates/rango/' directory. Did you put it in the right place? Did you complete the exercises?{FAILURE_FOOTER}")
    ...
    def test_restricted_template_inherits(self):
        ...
        self.assertTrue(re.search(full_title_pattern, content),
                        f"{FAILURE_HEADER}The <title> of the response for 'rango:restricted' is not correct. Check your restricted.html template, and try again.{FAILURE_FOOTER}")
        self.assertTrue(re.search(block_title_pattern, template_str),
                        f"{FAILURE_HEADER}Is restricted.html using template inheritance? Is your <title> block correct?{FAILURE_FOOTER}")
    ...
    def test_bad_add_page(self):
        ...
        self.assertEqual(response.status_code, 302,
                         f"{FAILURE_HEADER}When not logged in and attempting to add a page, we should be redirected. But we weren't. Check your add_page() implementation.{FAILURE_FOOTER}")
        self.assertTrue(response.url.startswith(reverse('rango:login')),
                        f"{FAILURE_HEADER}When not logged in and attempting to add a page, we should be redirected to the login page. But we weren't. Check your add_page() implementation.{FAILURE_FOOTER}")
    ...
    def test_bad_add_category(self):
        ...
        self.assertEqual(response.status_code, 302,
                         f"{FAILURE_HEADER}When attempting to add a category when not logged in, we weren't redirected when we should be. Check your add_category() implementation.{FAILURE_FOOTER}")
        self.assertTrue(response.url.startswith(reverse('rango:login')),
                        f"{FAILURE_HEADER}When attempting to add a category when not logged in, we weren't redirected to the login view. Check your add_category() implementation, and try again.{FAILURE_FOOTER}")
    ...
    def test_good_add_page(self):
        ...
        self.assertTrue('Add a Page' in content,
    ...
    def test_good_add_category(self):
        ...
        self.assertEqual(response.status_code, 200,
                         f"{FAILURE_HEADER}When adding a category (when logged in), we didn't get a HTTP 200 response. Please check your add_category() view.{FAILURE_FOOTER}")

        content = response.content.decode()
        self.assertTrue('Add a Category' in content,
                        f"{FAILURE_HEADER}When adding a category (when logged in), we didn't see the page we expected. Please check your add_category() view.{FAILURE_FOOTER}")
    ...
    def test_add_category_link(self):
        ...
        self.assertTrue(reverse('rango:add_category') not in content,
                        f"{FAILURE_HEADER}The Add Category link was present on Rango's homepage when a user is not logged in. This shouldn't be the case! Please check your base.html template.{FAILURE_FOOTER}")
        ...
        self.assertTrue(reverse('rango:add_category') in content,
                        f"{FAILURE_HEADER}The Add Category link was not present on Rango's homepage when the user was logged in. It should be visible! Please check base.html.{FAILURE_FOOTER}")
    ...
    def test_add_page_link(self):
        ...
        content = self.client.get(
            reverse('rango:show_category', kwargs={'category_name_slug': 'python'})).content.decode()

        self.assertTrue(reverse('rango:add_page', kwargs={'category_name_slug': 'python'}) not in content,
        ...
        self.assertTrue(reverse('rango:add_page', kwargs={'category_name_slug': 'python'}) in content,
